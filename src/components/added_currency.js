...
const AddedCurrency = ({coin, amountOwned, onChangeAmountOwned, removeCoin}) => {
    return [
        ...
        <div key={`${coin.name}-total`}><div>{(Number(coin.price_usd) * Number(amountOwned)).toLocaleString('en', { style: 'currency', currency: 'USD'})}</div><button onClick={() => removeCoin(coin.id)}>Remove</button></div>
    ]
};
